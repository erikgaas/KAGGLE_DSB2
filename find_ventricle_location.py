...
def calc_stupid_intersection(p1, p2, p3, p4):
    #Kill me
    p1 = list(reversed([p1[0], p1[1]]))
    p2 = list(reversed([p2[0], p2[1]]))
...
def draw_center_for_check(dcm_path, id, sax, point, points, geom, dcm_ext):
    ...
    center = calc_stupid_intersection([points[1], points[0]], 
        [points[3], points[2]], 
        [points[5], points[4]],
        [points[7], points[6]])
            ...
            if el != ch2_el and el != ch4_el:
                ...
                try:
                    ...
                    if debug == 1:
                        ...
                        curr_paths = glob.glob(str(os.sep).join(my_path[0:-1] + ['*']))
                        for each_file in curr_paths:
                            dcm_file_ext = each_file.split(os.sep)[-1].split('.')[0]
